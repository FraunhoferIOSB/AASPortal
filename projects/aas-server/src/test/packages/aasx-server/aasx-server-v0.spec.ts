    ...
    it('can open a file', async function () {
        jest.spyOn(http, 'request').mockImplementation((options, callback) => {
            ...
            stream.push(JSON.stringify({
                "aaslist": [
                    "0 : ExampleMotor : [IRI] http://customer.com/aas/9175_7013_7091_9168 : ",
                ]
            }));
            stream.push(null);
            stream.statusCode = 200,
        ...
        await expect(
            server.openFileAsync(aasEnvironment.assetAdministrationShells[0],
                selectElement(aasEnvironment, 'Documentation', 'OperatingManual', 'DigitalFile_PDF')!)).resolves.toBeTruthy();
    });
